...
class Py3status:
    ...
    regex = re.compile(r"(?:\$|USD) ?(?P<buy>\d{1,2}[.]\d{1,2})[.\-/]"
                       r"(?P<sell>\d{1,2}[.]\d{1,2})")
    ...
    def usd_currency(self):
        ...
        values = (reduce(lambda x, y: x + float(y[0]), res, 0) / len(res),
                  reduce(lambda x, y: x + float(y[1]), res, 0) / len(res),
